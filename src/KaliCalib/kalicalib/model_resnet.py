...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = torch.cat([x, x3_resized.detach()], dim = 1)
        #x = self.relu(self.dec1(x))

        x = self.relu(self.deconv2(x))
        x2_resized = F.interpolate(x2, size=(x.size(2), x.size(3)), mode='bilinear', align_corners=False)
        x = torch.cat([x, x2_resized.detach()], dim = 1)
        #x = self.relu(self.dec2(x))

        x = self.relu(self.deconv3(x))
        x_adjusted = x[:,:,:-1]
        x1_resized = F.interpolate(x1, size=(x_adjusted.size(2), x_adjusted.size(3)), mode='bilinear', align_corners=False)
        x = torch.cat([x_adjusted, x1_resized.detach()], dim = 1)
        #x = self.dec3(x)
